...
def subquadratic_multiply(x, y):
  ...
  if x.decimal_val <= 1 and y.decimal_val <= 1:
    ...
    add1 = BinaryNumber(x_left.decimal_val + x_right.decimal_val)
    add2 = BinaryNumber(y_left.decimal_val + y_right.decimal_val)
    left_mult_middle = subquadratic_multiply(add1,add2).decimal_val
    ...
    #decimal
    middle = left_mult_middle - left_middle - right_middle
...
def print_results(results):
  print(tabulate.tabulate(results,
              headers=['number','quad time', 'subquad time'],
              floatfmt=".3f",
              tablefmt="github"))
